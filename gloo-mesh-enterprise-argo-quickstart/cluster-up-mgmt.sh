...
k3d cluster create --servers 1 --network k3d-$k3d_cluster_name --api-port $k3d_cluster_port -p "$k3d_http_port:80@server[0]" -p "$k3d_https_port:443@server[0]" -p "9900:9900@server[0]" --no-lb --k3s-server-arg '--no-deploy=traefik' $k3d_cluster_name
